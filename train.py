...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_path", type=str, default='data/celeba/10client/train/', help="dataset path")
    parser.add_argument("--pretrained_weights", type=str, default='checkpoints/IMDB-WIKI_292_0.692704.pth', help="if specified starts from checkpoint model")
    ...
    # dataset_name = ''
    # for client in range(5, 10):
    client = 0
    print('---------'+str(client)+'---------')
    epochs = opt.epochs
    batch_size = opt.batch_size
    # train/0
    dataset_path = [os.path.join(opt.dataset_path, str(client), '0'), os.path.join(opt.dataset_path, str(client), '1')]
    # dataset_path = [os.path.join(opt.dataset_path, str(client))]
    ...
    for epoch in range(epochs):
        ...
        for i, image in enumerate(train_dataloader):
            ...
            log_str = "\r---- [Client %d, Epoch %d/%d, Step %d/%d] loss: %.6f----" % (client, epoch, epochs, i, len(train_dataloader), loss.item())
        ...
        if loss.item() < min_loss:
            min_loss = loss.item()
            torch.save(model.state_dict(), f'checkpoints/celeba%d/%d_%.6f.pth' % (client, epoch, loss.item()))
        ...
        with torch.no_grad():
            ...
            plt.title(str(epoch))
            plt.imshow(gen_img)
            plt.savefig(f'output/celeba%d/%d_%.6f.png' % (client, epoch, total_loss))
            # plt.show()
            plt.close()
